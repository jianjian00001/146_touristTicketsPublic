...
var content = (actions = {
    selectAll() { // 获取所有数据
        return new Promise((resolve, reject) => {
            if (content.state.status == 0) {
                ...
                httpGet("/jingdianxinxi/selectAll").then(
                    (res) => {
                        if (res.code == 0) {
    ...
    findById(id) {
        return new Promise((resolve, reject) => {
            httpGet("/jingdianxinxi/findById", { id }).then((res) => {
                if (res.code == 0) {
                    resolve(res.data);
                } else {
                    reject(res.msg);
    ...
    filter(data) {
        return new Promise((resolve, reject) => {
            httpPost("/jingdianxinxi/filter", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    query(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/jingdianxinxi/selectPages", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    update(data) {
        // 更新数据
        return new Promise((resolve, reject) => {
            httpPost("/jingdianxinxi/update", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
    ...
    insert(data) {
        // 插入数据
        return new Promise((resolve, reject) => {
            httpPost("/jingdianxinxi/insert", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
    ...
    checkForm(data) {
        return new Promise((resolve, reject) => {
            ...
            rules.jingdianbianhao = [{ required: true, message: "请填写景点编号" }];

            rules.jingdianmingcheng = [{ required: true, message: "请填写景点名称" }];

            rules.suoshudiqu = [{ required: true, message: "请填写所属地区" }];

            rules.jingquxingji = [{ required: true, message: "请填写景区星级" }];

            rules.tupian = [{ required: true, message: "请填写图片" }];

            rules.xiangqing = [{ required: true, message: "请填写详情" }];
    ...
    delete(data) {
        // 删除数据
        return new Promise((resolve, reject) => {
            httpPost("/jingdianxinxi/delete", isArray(data) ? data : [data]).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
