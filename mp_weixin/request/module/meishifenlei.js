...
var content = (actions = {
    selectAll() { // 获取所有数据
        return new Promise((resolve, reject) => {
            if (content.state.status == 0) {
                ...
                httpGet("/meishifenlei/selectAll").then(
                    (res) => {
                        if (res.code == 0) {
    ...
    findById(id) {
        return new Promise((resolve, reject) => {
            httpGet("/meishifenlei/findById", { id }).then((res) => {
                if (res.code == 0) {
                    resolve(res.data);
                } else {
                    reject(res.msg);
    ...
    filter(data) {
        return new Promise((resolve, reject) => {
            httpPost("/meishifenlei/filter", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    query(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/meishifenlei/selectPages", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    update(data) {
        // 更新数据
        return new Promise((resolve, reject) => {
            httpPost("/meishifenlei/update", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
    ...
    insert(data) {
        // 插入数据
        return new Promise((resolve, reject) => {
            httpPost("/meishifenlei/insert", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
    ...
    checkForm(data) {
        return new Promise((resolve, reject) => {
            ...
            rules.fenleimingcheng = [
                { required: true, message: "请填写分类名称" },
                { validator: rule.checkRemote, message: "内容重复了", checktype: "insert", module: "meishifenlei", col: "fenleimingcheng", trigger: "blur" },
            ];
    ...
    delete(data) {
        // 删除数据
        return new Promise((resolve, reject) => {
            httpPost("/meishifenlei/delete", isArray(data) ? data : [data]).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
