...
const readStore = require("./jingdianxinxi");
const app = getApp();
...
var content = (actions = {
    selectAll() { // 获取所有数据
        return new Promise((resolve, reject) => {
            if (content.state.status == 0) {
                ...
                httpGet("/menpiao/selectAll").then(
                    (res) => {
                        if (res.code == 0) {
    ...
    findById(id) {
        return new Promise((resolve, reject) => {
            httpGet("/menpiao/findById", { id }).then((res) => {
                if (res.code == 0) {
                    resolve(res.data);
                } else {
                    reject(res.msg);
    ...
    filter(data) {
        return new Promise((resolve, reject) => {
            httpPost("/menpiao/filter", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    query(data) {
        // 分页查询数据
        return new Promise((resolve, reject) => {
            httpPost("/menpiao/selectPages", data).then(
                (res) => {
                    if (res.code == 0) {
                        resolve(res);
                    } else {
    ...
    update(data) {
        // 更新数据
        return new Promise((resolve, reject) => {
            httpPost("/menpiao/update", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
    ...
    insert(data) {
        // 插入数据
        return new Promise((resolve, reject) => {
            httpPost("/menpiao/insert", data).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
    ...
    delete(data) {
        // 删除数据
        return new Promise((resolve, reject) => {
            httpPost("/menpiao/delete", isArray(data) ? data : [data]).then(
                (res) => {
                    if (res.code == 0) {
                        if (content.state.status == 0) {
                            content.dispatch("selectAll");
        ...
        return new Promise((resolve, reject) => {
            readStore
                ...
                .then((data) => {
                    ...
                    var autoText = ["jingdianxinxiid", "jingdianbianhao", "jingdianmingcheng", "suoshudiqu", "tupian"];
                    for (var txt of autoText) {
                        obj[txt] = data[txt];
                    }
                    obj.jingdianxinxiid = id;
